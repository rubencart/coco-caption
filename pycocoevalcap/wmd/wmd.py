...
class WMD:
    ...
    def __init__(self):
        with open(os.path.join(os.path.abspath(os.path.dirname(__file__)), 'data', 'stopwords.txt'), 'rb') as f:
            self.stop_words = set(f.read().decode('utf-8').strip().split(
                ' '))  # Stop words were taken from NLTK nltk.stopwords.words('english')
        self.model = gensim.models.KeyedVectors.load_word2vec_format(
            os.path.join(os.path.abspath(os.path.dirname(__file__)), 'data', 'GoogleNews-vectors-negative300.bin'),
            binary=True)
        self.sigma = 1.0
